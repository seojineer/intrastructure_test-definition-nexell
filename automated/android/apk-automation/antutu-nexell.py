...
class ApkRunnerImpl(ApkTestRunner):
    def __init__(self, config):
        ...
        self.config['apk_file_name'] = "antutu-benchmark-6-0-4.apk"
        ...
        for item in test_items:
            ...
            while not found_view:
                ...
                if score_view:
                    ...
                    try:
                        score = int(score)
                        if str(item) == '3D' and score > 800:
                            self.report_result('antutu6-%s' % item.lower(), 'pass', score, 'points')
                        elif str(item) == 'UX' and score > 11000:
                            self.report_result('antutu6-%s' % item.lower(), 'pass', score, 'points')
                        elif str(item) == 'CPU' and score > 10500:
                            self.report_result('antutu6-%s' % item.lower(), 'pass', score, 'points')
                        elif str(item) == 'RAM' and score > 2600:
                            self.report_result('antutu6-%s' % item.lower(), 'pass', score, 'points')
                        else:
                            self.report_result('antutu6-%s' % item.lower(), 'fail', score, 'points')
